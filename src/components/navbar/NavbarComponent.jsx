  ...
  return (
    <div className='relative h-20 flex items-center justify-around flex-nowrap  gap-10 max-sm:gap-0 max-sm:justify-between  bg-white shadow dark:bg-gray-950 dark:shadow-white'>
      ...
      <div className='flex items-center gap-4  shrink-0'>    
        <ul className={`max-md:bg-indigo-400 backdrop-blur-sm rounded-xl max-md:items-start flex items-center gap-4 absolute md:static  md:w-auto  flex-col md:flex-row p-4 md:p-0  transition-all duration-100 ` + (isMenuOpen ? '-top-90 left-0 w-full' : ' top-20 left-0 w-full overflow-hidden p-5 ')}>
      ...
      isActive
        ...
        : "text-indigo-600"
          ...
          onClick={menubarToggle} end> <li className={` ${theme ==="dark" ? 'text-white' : ' text-black'} hover:text-indigo-600 cursor-pointer`}>
              HOME</li></NavLink> 
      ...
      isActive
        ? "text-indigo-500 font-semibold border-b-2 border-indigo-500"
        : "text-indigo-600"
       }`} onClick={menubarToggle}> <li className={` ${theme ==="dark" ? 'text-white' : ' text-black'} hover:text-indigo-600 cursor-pointer`}>
              About</li></NavLink> 
      ...
      isActive
       ...
       <button onClick={()=>setIsMenuOpen(!isMenuOpen)} className={`md:hidden px-2 py-2 rounded-full transition duration-300 cursor-pointer hover:bg-indigo-500 shrink-0' + ${theme ==="dark" ? 'text-white' : ' text-black'}`}>
