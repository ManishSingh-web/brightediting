...
const  BeforeAfterSlider = ({beforeImage,afterImage}) => {
  const containerRef = useRef(null);
    ...
    return (
      <div className="w-300 h-150 max-md:h-100 max-md:w-full max-sm:h-70  flex justify-center items-center  rounded-2xl ">
        <div
          ref={containerRef}
          onClick={(e) => handleMove(e.clientX)}
          className="relative w-full max-w-10xl h-full  rounded-xl shadow-2xl overflow-hidden select-none"
        >
          <img
            src={afterImage}
            alt="after"
          ...
          >
            <div className="absolute top-1/2 left-1/2 w-20 h-20 bg-amber-500 border-4 border-amber-300 text-center items-center rounded-full -translate-x-1/2 -translate-y-1/2 cursor-e-resize">
             <h3 className=" text-white mt-2"> 24 h Turnaround</h3>
            </div>
          </div>
