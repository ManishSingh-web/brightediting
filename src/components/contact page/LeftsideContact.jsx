...
const LeftsideContact = () => {
  ...
  return (
    ...
    className='flex flex-col items-start mt-10 font-sans px-10 py-10 rounded-2xl bg-gray-200 dark:bg-gray-900  min-w-0'>
        <h2 className='text-3xl font-bold mb-6 text-center text-indigo-600'>CONTACT INFORMATION </h2>
        <p className='text-gray-400 text-center text-lg' style={{fontFamily:"system-ui"}}>fill out the contact form or contact us directly using <br/>information below</p>
       {
        data.map((item)=>{
           ...
           return(
            <div key={item.id} className='flex items-center mt-4 font-sans  p-4 rounded-lg bg-gray-300 dark:bg-gray-700 flex-wrap max-sm:flex-col min-w-0'>
                                <div className='text-4xl text-indigo-600  px-2 py-2  rounded-2xl '>{item.icon}</div>
